...
namespace ConwaysGameOfLifeTest
{
    public class UniverseTest
    {
            ...
            ConsoleColor eighthCellColour, State ninthCellState, ConsoleColor ninthCellColour)
        {
            ...
            Assert.Equal(firstCellColour, seed.SeedGrid[0, 0].GetColour());
            Assert.Equal(secondCellState, seed.SeedGrid[0, 1].State);
            Assert.Equal(secondCellColour, seed.SeedGrid[0, 1].GetColour());
            Assert.Equal(thirdCellState, seed.SeedGrid[0, 2].State);
            Assert.Equal(thirdCellColour, seed.SeedGrid[0, 2].GetColour());

            Assert.Equal(fourthCellState, seed.SeedGrid[1, 0].State);
            Assert.Equal(fourthCellColour, seed.SeedGrid[1, 0].GetColour());
            Assert.Equal(fifthCellState, seed.SeedGrid[1, 1].State);
            Assert.Equal(fifthCellColour, seed.SeedGrid[1, 1].GetColour());
            Assert.Equal(sixthCellState, seed.SeedGrid[1, 2].State);
            Assert.Equal(sixthCellColour, seed.SeedGrid[1, 2].GetColour());

            Assert.Equal(seventhCellState, seed.SeedGrid[2, 0].State);
            Assert.Equal(seventhCellColour, seed.SeedGrid[2, 0].GetColour());
            Assert.Equal(eighthCellState, seed.SeedGrid[2, 1].State);
            Assert.Equal(eighthCellColour, seed.SeedGrid[2, 1].GetColour());
            Assert.Equal(ninthCellState, seed.SeedGrid[2, 2].State);
            Assert.Equal(ninthCellColour, seed.SeedGrid[2, 2].GetColour());
        }
        ...
        public void Corner_Cell_Should_Return_Orientation_Of_Corner(int row, int column, PositionType expectedPosition)
        {
            ...
            var actualPosition = cell.GetPositionType();
        ...
        public void Alive_Middle_Cell_Which_Has_No_Live_Neighbours_In_A_3_By_3_Universe_Should_Display_All_Dead_Cells_In_Next_Generation()
        {
            ...
            Assert.Equal(Constants.Dead, cell.GetColour());
        }
